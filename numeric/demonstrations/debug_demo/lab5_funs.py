...
print("importing from debug_demo")
import pdb
...
class Integrator:
    ...
    def __str__(self):
        out = (
            "integrator instance with attributes initvars, timevars,uservars, "
            + "adaptvars"
        )
        return out
    ...
    def rkckODE5(self, yold, timeStep, deltaT):
        ...
        for i in np.arange(5):
            ...
            # pdb.set_trace()
            derivArray[i + 1, :] = self.derivs5(
                y + deltaT * bsum, timeStep + a[i] * deltaT
            )[:]
...
class Integ53(Integrator):
    def set_yinit(self):
        ...
        self.yinit = np.array([self.initvars.whiteconc, self.initvars.blackconc])
