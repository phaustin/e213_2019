...
def param():
    ...
    Av = 5.0e-2  # vertical eddy viscosity (m^2/s)
    H = 500.0  # depth (m)
    rho = 1.0e3  # density of water (kg/m^3)
    latitude = 45.0  # for calculating parameters of the beta-plane (deg)
    ...
    earth_radius = 6.4e6  # radius of the Earth (m)
...
def numer_init():
    ...
    dx = 1.0 / (nx - 1)  # size of non-dimensional grid points
    ny = int(1.0 / dx + 1)  # number of points in y-direction
    ...
    coeff = 1.0  # relaxation coefficient
...
def vis(psi, nx, ny):
    ...
    # indexing version
    si = 1
    ei = nx - 1  # start and end i indices
    sj = 1
    ej = ny - 1  # start and end j indices
    visc[si:ei, sj:ej] = (
        psi[si + 1 : ei + 1, sj:ej]
        + psi[si - 1 : ei - 1, sj:ej]
        + psi[si:ei, sj + 1 : ej + 1]
            ...
            for i in range(1, nx - 1):
                for j in range(1, ny - 1):
                    r[i, j] = rhs[i, j] * dx * dx * 0.25 - (
                        (chi[i + 1, j] + chi[i, j + 1] + chi[i - 1, j] + chi[i, j - 1])
                        * 0.25
                        - chi[i, j]
                    )
