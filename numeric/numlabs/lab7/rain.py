...
def make_graph(u, h, dt, n_time):
    ...
    the_title = fig.text(
        0.25, 0.95, "Results from t = %.3fs to %.3fs" % (0, dt * n_time)
    )
    ...
    cNorm = colors.Normalize(vmin=0, vmax=1.0 * n_time)
    cNorm_inseconds = colors.Normalize(vmin=0, vmax=1.0 * n_time * dt)
