...
def leap_frog(u, v, h, g, H, f, dt, dx, n_grid):
    ...
    for pt in xrange(1, n_grid - 1):
        u.next[pt] = u.prev[pt] + 2*dt*(f*v.now[pt] - (g/dx)*(h.now[pt+1] - h.now[pt]))
        v.next[pt] = v.prev[pt] - 2*dt*(f*u.now[pt])
        h.next[pt] = h.prev[pt] - 2*dt*(H/dx)*(u.now[pt] - u.now[pt-1])
...
def first_time_step(u, v, h, g, H, f, dt, dx, n_grid):
    ...
    for pt in xrange(1, n_grid - 1):
        u.now[pt] = u.prev[pt] + dt*(f*v.prev[pt] - (g/dx)*(h.prev[pt+1] - h.prev[pt]))
